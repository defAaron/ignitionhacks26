...
- **The product:** baio is autocomplete for drawing — sketch a page, watch it bloom in like wet ink, Seal it, Frame the space into a working website.
...
## Tagline (~140 chars, HackHub listing / social)

> Autocomplete for drawing: sketch a page, watch it bloom in like wet ink — real, editable components, sealed and framed into a working website.
...
baio is autocomplete for drawing. Sketch a rough page — boxes, glyph letters, scribbles — and a fine-tuned 2B model we trained this weekend substitutes real, editable components exactly where you drew them, blooming in like watercolor. Seal a page, Frame the space, and you leave with a working website.

## Medium description (~120 words)

Our essays have autocomplete; our thoughts don't. Ideas fade faster than pens move — so baio brings autocomplete to the canvas. Draw naturally: every enclosed shape gets crisp, a single letter in a box adds function (`b` → button, `n` → navbar, `p` → a new page), words and colors become labels and fills, and sketched skeletons become full diagrams — up to the 118-element periodic table. Suggestions bloom in as translucent watercolor and dry into editable vector structure on accept; low confidence means the model abstains and your ink stays ink. The decision-maker is a Qwen3.5-2B model we fine-tuned on FreeSolo for under $0.25 — beating the Gemini baseline 96.7% to 75.0% on op accuracy. Seal a page and Frame the plane: Claude turns the finished wireframe into a downloadable, working website (one file or a multi-page site).
...
You draw on real paper-feeling canvas. Every enclosed shape becomes a crisp shape, filled with the color you shaded it. Semantics are opt-in via glyphs — a single letter alone in a box (`b` button, `n` navbar, `f` form, `i` image, `v` video, `p` a new page) — so plain shapes stay plain and there are never surprise components. Words become labels ("Login"), colors become fills, theme words become gradients ("rainbow"). Decoratives (night skies, wave dividers, auroras) and six diagram types (bar, pie, Venn, timeline, atomic structure, the full periodic table) render procedurally and seeded. Pages sit on an infinite plane; layers spawn on overlap; photos drop into any drawn enclosure; arrows between objects become logic wires. When a page is done, Seal sends the wireframe to Claude and returns a complete, responsive, interactive website (HTML plus a project zip). Frame stitches every sealed page into a linked multi-page site.
