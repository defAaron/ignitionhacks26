...
import { FRAME_SPACE_SYSTEM_PROMPT, buildFrameSpaceUserMessage } from '@/lib/frame/space-prompt'
import { parseFrameFiles } from '@/lib/frame/app-types'
import type { FrameSpaceRequest, FrameSpaceResponse } from '@/lib/frame/space-types'
...
/**
 * A whole multi-page project is many files; 64k leaves headroom for the model's
 * (adaptive) thinking plus every file's source. Outputs this large require
 * streaming to avoid request timeouts, so the call streams SSE and the route
 * accumulates the text server-side.
 */
const MAX_TOKENS = 64_000

function fail(reason: string, status = 500): NextResponse<FrameSpaceResponse> {
...
export async function POST(req: Request): Promise<NextResponse<FrameSpaceResponse>> {
  ...
  try {
    body = (await req.json()) as FrameSpaceRequest
  } catch {
    return fail('invalid JSON body', 400)
  }
  if (!Array.isArray(body.pages) || body.pages.length === 0) {
    return fail('pages must be a non-empty array', 400)
  }
  // Phase 2 may omit connections; the generator then falls back to name-matching.
  const connections = Array.isArray(body.connections) ? body.connections : []

  const { message, images } = buildFrameSpaceUserMessage(body.pages, connections)
  ...
  try {
    res = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        system: FRAME_SPACE_SYSTEM_PROMPT,
  ...
  if (stopReason === 'max_tokens') {
    return fail('the site was cut off (max_tokens reached) — try fewer pages')
  }
